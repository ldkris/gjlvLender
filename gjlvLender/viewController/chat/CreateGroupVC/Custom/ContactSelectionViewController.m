...
@property (strong, nonatomic) NSMutableArray *selectedContactsIndex;
@end
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        _selectedContactsIndex = [NSMutableArray array];
...
- (void)viewDidLoad
{
    ...
    backButton.accessibilityIdentifier = @"icon_back";
...
//    [self.view addSubview:self.footerView];
    ...
    [_doneButton setBackgroundImage:[UIImage imageNamed:@"t_btn_bg"] forState:UIControlStateNormal];
    [_doneButton setFrame:CGRectMake(0, 0, 80, 30)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.mulChoice) {
        if (![self.selectedContacts containsObject:object])
        {
            ...
            [self.selectedContactsIndex addObject:[tableView cellForRowAtIndexPath:indexPath]];
            [self reloadFooterView];
        }else{
            [self.selectedContacts removeObject:object];
             [self.selectedContactsIndex removeObjectAtIndex:indexPath.row];
            [self reloadFooterView];
    ...
    for ( BaseTableViewCell *cell in tableView.visibleCells) {
        if ([self.selectedContactsIndex containsObject:cell]) {
            [cell.selectImgView setImage:[UIImage imageNamed:@"s_selected"]];
        }
        else
        {
            [cell.selectImgView setImage:[UIImage imageNamed:@"s_unselected"]];
        }
    }
}
