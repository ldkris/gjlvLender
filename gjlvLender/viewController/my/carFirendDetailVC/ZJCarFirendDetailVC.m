...
#import "ZJInPutVC.h"
@interface ZJCarFirendDetailVC ()<UITableViewDataSource,UITableViewDelegate>

@property (weak, nonatomic) IBOutlet UITableView *mInfoTbaleView;
@property(nonatomic,retain)NSArray *mDataSoure;
@end

@implementation ZJCarFirendDetailVC{

    NSString *_headImageStr;
    ZJCarFirendsModel *mInfoModel;
}
...
-(void)getInfo{
    if (self.mCarFirendId && self.mCarFirendId.length>0) {
        [[ZJNetWorkingHelper shareNetWork]getUserInfoDetail:@{@"leaderId":ZJ_UserID,@"userId":self.mCarFirendId } SuccessBlock:^(id responseBody) {
            NSError *error;
            ZJCarFirendsModel *userModel = [MTLJSONAdapter modelOfClass:[ZJCarFirendsModel class] fromJSONDictionary:responseBody error:&error];
            mInfoModel = userModel;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == 0) {
        ...
        cell.btn_change.hidden = YES;
        [cell.mIMG sd_setImageWithURL:[NSURL URLWithString:mInfoModel.mHeadImgUrl] placeholderImage:[UIImage imageNamed:@"dd_def.png"] options:EMSDWebImageRefreshCached];
        return cell;
    }
    [tableView registerNib:[UINib nibWithNibName:@"ZJCFDetalCell" bundle:[NSBundle mainBundle]] forCellReuseIdentifier:@"ZJCFDetalCell"];
    ZJCFDetalCell *cell = [tableView dequeueReusableCellWithIdentifier:@"ZJCFDetalCell"];
    cell.mLab_Title.text = self.mDataSoure[indexPath.row];
//    cell.mLab_subTitle.text = self.mDataSoure[indexPath.row];
    [cell loadDataIndexpath:indexPath model:mInfoModel];
    return cell;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    ZJCFDetalCell *cell = [tableView cellForRowAtIndexPath:indexPath];
